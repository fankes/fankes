...
Companion to [Moshi](https://github.com/square/moshi) with more practical features.

**Recommendation** ⭐⭐⭐⭐⭐

If you are using Moshi for JSON parsing and do not want the default obfuscation rule for `moshi-kotlin-codegen` to retain class names, it will be able to no longer depend on R8 when Android projects enable
`Class.forName` looks for the adapter class generated by `moshi-kotlin-codegen`, so as to achieve complete obfuscation of entity class names and fields, improve code security and reduce APK size.

[Click here to view](https://github.com/HighCapable/moshi-companion)
